		...
		if($_POST['type_titre']==1){
			...
			*/
			if($_POST['titre']!="" && $_POST['text_article']!=""){
				...
				$db->execute("INSERT INTO cms_footer (ID_PARENT,ID_ELEMENT,ID_LANG,ID_UNIVERS,TYPE,ORDRE,TITLE,TEXT,META_DESC,META_KEY) VALUES (".$_POST['parent'].",0,".$_SESSION['langue'].",".$_SESSION['univers'].",".$type.",".($count+1).",'".addslashes($_POST['titre'])."','".addslashes($_POST['text_article'])."','".addslashes($_POST['meta_desc'])."','".addslashes($_POST['meta_key'])."')");
		...
		elseif($_POST['type_titre']==2){
			...
			*/
			if($_POST['type_select']=='externe-1' && $_POST['external_url']==''){
				...
				$db->execute("INSERT INTO cms_footer (ID_PARENT,ID_ELEMENT,ID_LANG,ID_UNIVERS,TYPE,TITLE,ORDRE) VALUES (".$_POST['parent'].",'".addslashes($id_element)."',".$_SESSION['langue'].",".$_SESSION['univers'].",".$type.",'".addslashes($_POST['titre_rac'])."',".($count+1).")");
		...
		if($_POST['type_titre']==1){
			...
			*/
			if($_POST['titre']!="" && $_POST['text_article']!=""){
				...
				$db->execute("UPDATE cms_footer SET ID_ELEMENT = 0, TYPE = 0, TITLE = '".addslashes($_POST['titre'])."', TEXT = '".addslashes($_POST['text_article'])."' , META_DESC = '".addslashes($_POST['meta_desc'])."', META_KEY = '".addslashes($_POST['meta_key'])."' WHERE ID = ".$_POST['id']);
		...
		elseif($_POST['type_titre']==2){
			...
			*/
			if($_POST['type_select']=='externe-1' && $_POST['external_url']==''){
				...
				$db->execute("UPDATE cms_footer SET ID_ELEMENT = '".addslashes($id_element)."', TYPE = ".$type." , TITLE = '".addslashes($_POST['titre_rac'])."' WHERE ID = ".$_POST['id']);		
...
if($action==''){
	...
	<script>
		$(document).ready(function(){
			'.auto_help("Pied de page","<a href='#' id='titre_guide'>Voir le guide &quot;Pied de page &quot;</a>",'titre_guide','faq_view.php?id_aide=61&id_cat=8').'
		});
	</script>
	';
...
?>
	...
	<form method="post"  name="form_nav" action="nav_footer.php?action=<?echo $action;?>_r">
		...
		<div class="clear"></div>
		<div id="step1_admin" <?echo add_help("Pied de page","<a href='#' id='titre_guide'>Voir le guide &quot;Comment gérer le Pied de page&quot;</a>",'titre_guide','faq_view.php?id_aide=57&id_cat=8'); ?>>
			<div>Je souhaite</div>
			<div style="margin-top:15px;">
				<input type="radio" name="type_titre" id="type_titre1" value="1" <?if($form['TYPE']=="" || $form['TYPE']==0){ echo "checked='checked'";}?> class='check_no_border'> Créer une nouvelle page
			</div>
			<div style="margin-top:15px;">
				<input type="radio" name="type_titre" id="type_titre2" value="2" <?if($form['TYPE']!="" && $form['TYPE']!=0){ echo "checked='checked'";}?> class='check_no_border'> <span id="type_titre3">Faire un raccourci vers un élément existant (arborescence, formulaires, galeries photos, vidéos).</span>
			</div>
			...
			<div style="margin-top:10px;">
				...
				<select name="parent" class="select_class" style="width:400px;">
					...
					$req=$db->query("select * from cms_footer where ".$req_plus." ID_PARENT = 0 AND ID_LANG = ".$_SESSION['langue']." AND ID_UNIVERS = ".$_SESSION['univers']." order by ORDRE ASC");
					if(mysql_num_rows($req)!=0){
						while($don=mysql_fetch_array($req)){
		...
		<div id="step2_admin">
			...
			<div style="margin-top:5px;" <? echo add_help('Nom personnalisé',"Vous avez la possibilité de saisir un nom personnalisé qui sera repris sur le menu. Si vous laissez vide le champ , le titre du raccourci sera repris automatiquement. Par exemple un raccourc vers un article nommé 'Nous contacter' affichera 'Nous contacter' sur le site.");?>>
				<input name="titre_rac" type="text" value='<?echo $form['TITLE'];?>' id="titre_rac" style="width:350px;" />
			</div>
