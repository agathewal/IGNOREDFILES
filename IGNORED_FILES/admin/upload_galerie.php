...
*/
	$inF = fopen('log-telechargement.txt',"a+");
	fwrite($inF,'Téléchargement d\'un fichier'."\n");
...
// Settings
	if($_POST['ACTION']=="DIRECT" && is_numeric($_POST['ID']))$save_path="../images/galerie/".$_SESSION['dir_upload']."/".$_POST['ID'].'/';
	else $save_path = "../tmp/";			
...
// Validate file name (for our purposes we'll just remove invalid characters)
	...
	$tmp_file_name=$_FILES[$upload_name]['name'];
	$tmp_file_name=pathinfo($tmp_file_name);
	...
	fwrite($inF,'Téléchargement d\'un fichier'.$file_name.'-'.$_POST['ID']."\n");

// Validate that we won't over-write an existing file
	if (file_exists($save_path . $file_name)) {
	
		$file_info=pathinfo($file_name);		
		$file_name=$file_info['filename'].mt_rand(0, 100000).'.'.$file_info['extension'];
		fwrite($inF,'Téléchargement d\'un fichier'.$file_info['filename'].mt_rand(0, 100000).'.'.$file_info['extension']."\n");
...
// Process the file
	...
	*/
	...
	$thumb = PhpThumbFactory::create($_FILES[$upload_name]["tmp_name"]);  
	$thumb->resize(1024, 768);  
	
	
	if (!$thumb->save($save_path.$file_name)) {
		HandleError("File could not be saved.");
		exit(0);
	}else{
		fwrite($inF,'Téléchargement terminé '.$file_name.' pour la galerie ID '.$_POST['ID']."\n");
		
		$ordre=$db->countOf('cms_photo','ID_GALERIE='.$_POST['ID']);
		fwrite($inF,"INSERT INTO cms_photo (TITLE,ORDRE,ID_GALERIE,ID_USER,DT_CREATE,DT_MODIF,FILE) VALUES ('',".($ordre+1).",".$_POST['ID'].",".$_POST['USER_ID'].",".time().",".time().",'".$file_name."')");
		$db->execute("INSERT INTO cms_photo (TITLE,ORDRE,ID_GALERIE,ID_USER,DT_CREATE,DT_MODIF,FILE) VALUES ('',".($ordre+1).",".$_POST['ID'].",".$_POST['USER_ID'].",".time().",".time().",'".$file_name."')");
	}
